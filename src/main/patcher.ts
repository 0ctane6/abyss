...
if (!IS_VANILLA) {
    ...
    if (!IS_VESKTOP && !IS_EQUIBOP) {
        try {
            ...
            console.error("[Abyss] Failed to install host update hook", err);
        }
    }
    ...
    process.env.DATA_DIR = join(app.getPath("userData"), "..", "Abyss");
    ...
    console.log("[Abyss] Running in vanilla mode. Not loading Abyss");
}

console.log("[Abyss] Loading original Discord app.asar");
